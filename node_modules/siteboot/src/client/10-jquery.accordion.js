...
;(function($) {
    ...
    return this.each(function() {
          ...
          if ($node.find(o.next).length || $node.next(o.next).length) {
            if ($node.find('> a').length) {
              if (o.iconTrigger) {
                ...
                $node.find('> a').addClass("link trigger");
              }
            } else {
            ...
            if (o.lastChild && $node.find('> a').length) {$node.find('> a').addClass("link trigger");}
          }
      });
      function nodes(el) {
        var txt = [];
      ...
      if (o.activeLink && o.initShow) {
          var fullURL = window.location.href,
              relTo = (o.uri != 'full') ? o.uri.length : null,
              arrPath = fullURL.slice(relTo).split(o.splitUrl),
  ...
  $.fn.accordion.defaults = {
    ...
    uri : 'full', // 'full' if you use absolute paths. If you use relative paths in a navigation accordion, specify what they are relative to, e.g 'http://www.domain.com/' 
...
/*  ---
    ...
    /// Standard nested lists:
      ...
      // NEW in v.1.4.7.4: In a navigation accordion, we can use relative paths. We should specify what they are relative to, e.g uri: 'http://www.domain.com'.
      $("#container1").accordion({initShow: "#current", uri: "http://www.domain.com", splitUrl: '?'});    
